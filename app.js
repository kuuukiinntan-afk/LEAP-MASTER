...
function home(){
  const k=learnedCount(), total=W.length||2300, pct=total?Math.round(k/total*100):0;
  return `<section class="panel">
    <div class="hero glass">
      <div><div class="eyebrow">LEAP 2300</div><h1>NO LEAP<br>NO LIFE.</h1><p class="muted">2300語を、毎日少しずつ。</p></div>
      <button class="btn primary" data-go="test">テストを始める</button>
    </div>
    <div class="grid">
      <div class="card glass stat"><div class="label">覚えた単語</div><div class="val">${k}<small> / ${total}</small></div></div>
      <div class="card glass stat"><div class="label">苦手単語</div><div class="val">${weakCount()}</div></div>
      <div class="card glass stat"><div class="label">RP</div><div class="val">${S.rp}</div></div>
      <div class="card glass stat"><div class="label">正答率</div><div class="val">${S.tests?Math.round(S.correct/S.tests*100):0}<small>%</small></div></div>
    </div>
    <div class="card glass" style="margin-top:13px"><div class="between"><b>2300語の進捗</b><span class="muted">${pct}%</span></div><div class="progress"><i style="width:${pct}%"></i></div></div>
    <div class="quick">
      <div class="card glass" data-go="cards"><b>🃏 単語カード</b><p class="muted small">番号順・ランダム・苦手優先</p></div>
      <div class="card glass" data-go="training"><b>⚡ トレーニング</b><p class="muted small">苦手単語を集中練習</p></div>
      <div class="card glass" data-go="friends"><b>👥 フレンド</b><p class="muted small">友達を追加</p></div>
    </div>
  </section>`;
}
