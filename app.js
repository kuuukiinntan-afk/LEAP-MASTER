/* Optional online adapter for LEAP 2300.
   Fill the Firebase config in the UI settings to enable real-time friends/ranking/battle.
   This file is intentionally optional; the app remains fully usable offline without it. */
(() => {
  const key = "leap_firebase_config_v1";
  ...
  window.LEAP_SAVE_FIREBASE_CONFIG = async (config) => {
  ...
  api.createRoom = async ({roomId,opponentId,count,ownerId,rp}) => {
    if(!db) throw new Error("Firebase is not connected");
    await db.ref("rooms/"+roomId).set({ownerId,opponentId,count,status:"waiting",createdAt:firebase.database.ServerValue.TIMESTAMP,players:{[ownerId]:{rp}}});
    return roomId;
  };
  api.publishProfile = async ({id,name,rp,streak}) => {
    if(!db || !id) return;
    await db.ref("users/"+id).update({name:name||id,rp:rp||0,streak:streak||0,online:true,updatedAt:firebase.database.ServerValue.TIMESTAMP});
  };
  api.watchProfile = (id,cb) => db && db.ref("users/"+id).on("value",s=>cb(s.val()));
  api.getRanking = async (ids) => {
    if(!db) return [];
    const out=[]; for(const id of ids){const s=await db.ref("users/"+id).once("value");if(s.exists())out.push({id,...s.val()});} return out;
  };
  init();
})();
...
const menus = [
 ...
 ["battle","⚔️ オンライン対戦"],["ranking","🏆 ランキング"],["records","📅 学習記録"],["settings","⚙️ UI設定"]
];
...
function menu(){
  $("#sideNav").innerHTML=menus.map(([v,t])=>`<button class="menuItem ${S.view===v?"active":""}" data-view="${v}">${t}</button>`).join("")
    + `<div class="help glass"><b>LEAP 2300</b><br>${DATA_OK?"✅ 2300語読み込み済み":"❌ 単語データを確認してください"}<br><span class="small">ペット・ガチャ・集中モードを廃止して、ランク・対戦・UIカスタムを強化。</span></div>`;
...
function cards(){
  ...
  return `<section class="panel study">
    <div class="row"><h2>🃏 単語カード</h2><span class="small">No.${w.id} / ${deck.length}</span></div>
    <div class="controls">
    ...
    <div id="flashcard" class="flashcard ${flipped?"flipped":""}" tabindex="0" aria-label="単語カード。タップまたは左右スワイプで操作">
      <div class="flash-inner">
        <div class="flash-face"><span class="small">ENGLISH</span><div class="word">${esc(w.word)}</div><p>タップで裏返す</p></div>
        <div class="flash-face flash-back"><span class="small">日本語</span><div class="meaning">${esc(w.meaning)}</div><p>タップで表に戻す</p></div>
      </div>
    </div>
    <div class="cardHint">← 左スワイプ：次へ　｜　タップ：表裏　｜　右スワイプ：前へ →</div>
    <div class="actions"><button id="applyCard" class="primary">範囲を適用</button><button id="known" class="knownBtn">✓ 覚えた</button><button id="weak" class="weakBtn">★ 苦手</button><button id="nextCard">次へ</button></div>
  </section>`;
}
function list(){
...
function battle(){
 return `<section class="panel"><div class="row"><h2>⚔️ オンライン対戦</h2>${onlineStatus()}</div>
   <div class="battleHero"><div class="battleIllustration"><span>VS</span><i>✦</i></div><div><div class="eyebrow">LEAP BATTLE</div><h2>友達と10問勝負</h2><p class="small">同じ問題セットを解いて、正答数と回答スピードで勝負。</p></div></div>
   ...
   <button id="createBattle" class="primary full">対戦ルームを作る</button><div id="battleResult"></div>
 </section>`;
}
...
function ranking(){
 const local=[{name:"あなた",rp:S.rp,me:true},...S.friends.map(f=>typeof f==="string"?{name:f,rp:0}:{name:f.name||f.id||"Friend",rp:f.rp||0})].sort((a,b)=>b.rp-a.rp);
 ...
 return `<section class="panel"><div class="row"><h2>🏆 ランク</h2>${onlineStatus()}</div>
   ...
   <div class="rows">${local.map((x,i)=>`<div class="rankItem ${x.me?"me":""}"><span><b>${i+1}位</b>　${esc(x.name)}</span><strong>${x.rp} RP</strong></div>`).join("")}</div>
 </section>`;
}
function records(){
...
function themeButtons(){
 const themes=[['glass','Liquid Glass','透明感'],['midnight','Midnight','ダーク'],['paper','Paper','明るくシンプル'],['mint','Mint','爽やか']];
...
function settings(){
 return `<section class="panel"><h2>⚙️ UI設定</h2><p class="small">集中モードは削除しました。かわりに、LEAP全体の見た目を選べます。</p>
...
function home(){
 ...
 return `<section class="homegrid">
   <div class="hero panel"><div class="heroCopy"><div class="eyebrow">LEAP 2300</div><h1>単語力を、<br><span>ランク</span>に。</h1><p>2300語を覚えて、テストして、友達と競う。</p><div class="heroBtns"><button class="primary" data-go="cards">単語カードを始める</button><button data-go="test">テストする</button></div></div><div class="heroArt"><div class="orbit o1"></div><div class="orbit o2"></div><div class="bookArt"><div>LEAP</div><span>2300</span></div><i class="spark s1">✦</i><i class="spark s2">✧</i></div></div>
...
function bind(){
 ...
 if(fc){
   ...
   fc.addEventListener("touchend",e=>{const t=e.changedTouches[0],dx=t.clientX-touchStartX,dy=t.clientY-touchStartY;if(Math.abs(dx)>55&&Math.abs(dx)>Math.abs(dy)){swiped=true;if(dx<0){cardIndex++;}else{cardIndex=(cardIndex-1+deck.length)%deck.length;}flipped=false;render("cards")}},{passive:true});
   fc.addEventListener("keydown",e=>{if(e.key==="ArrowLeft"){cardIndex++;flipped=false;render("cards")}else if(e.key==="ArrowRight"){cardIndex=(cardIndex-1+deck.length)%deck.length;flipped=false;render("cards")}else if(e.key==="Enter"||e.key===" "){e.preventDefault();flipped=!flipped;render("cards")}});
 }
 $("#nextCard")?.addEventListener("click",()=>{cardIndex=(cardIndex+1)%deck.length;flipped=false;render("cards")});
 $("#known")?.addEventListener("click",()=>{const w=deck[cardIndex%deck.length];S.known[w.id]=true;S.weak[w.id]=false;cardIndex=(cardIndex+1)%deck.length;flipped=false;recordStudy(1);render("cards")});
 $("#weak")?.addEventListener("click",()=>{const w=deck[cardIndex%deck.length];S.weak[w.id]=true;S.known[w.id]=false;cardIndex=(cardIndex+1)%deck.length;flipped=false;recordStudy(1);render("cards")});
 $("#applyCard")?.addEventListener("click",()=>{let a=+$("#cf").value||1,b=+$("#ct").value||2300;deck=W.filter(w=>w.id>=Math.min(a,b)&&w.id<=Math.max(a,b));if($("#co").value==="random")deck=shuffle(deck);if($("#co").value==="weak")deck=[...deck.filter(w=>S.weak[w.id]),...deck.filter(w=>!S.weak[w.id])];if(!deck.length)deck=W.slice();cardIndex=0;flipped=false;render("cards")});
 ...
 $("#createBattle")?.addEventListener("click",async()=>{
   ...
   if(firebaseReady() && typeof window.LEAP_ONLINE_API.createRoom==="function"){
     try{await window.LEAP_ONLINE_API.createRoom({roomId:S.online.room,opponentId:opponent,count,ownerId:S.friendId,rp:S.rp});box.innerHTML=`<div class="battleRoom"><b>対戦ルーム ${esc(S.online.room)}</b><p class="small">相手が参加するまで待機中…</p></div>`;return}catch(e){console.error(e)}
   }
   box.innerHTML=`<div class="battleRoom"><b>ルームコード：${esc(S.online.room)}</b><p class="small">この画面はオンライン対戦UIまで完成。リアルタイム通信を有効にするにはFirebase等の接続が必要です。</p></div>`;
 });
