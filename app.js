...
function menu(){
  ...
  const items=[
    ...
    ["records","📅 学習記録"],["settings","⚙️ UI設定"]
  ];
  nav.innerHTML=items.map(([v,t])=>
    `<button type="button" class="menuItem ${S.view===v?"active":""}" data-view="${v}" onclick="window.LEAPRender('${v}');window.LEAPCloseMenu();return false;">${t}</button>`
...
function battle(){
  return `<section class="panel"><div class="row"><h2>⚔️ オンライン対戦</h2>${onlineStatus()}</div>
    ...
    <div class="notice">${firebaseReady()?"オンライン接続済み。ルームを作成するとフレンドへ招待が届きます。":"オンライン対戦を使うには、UI設定でFirebaseを接続してください。"}</div>
  </section>`;
}
...
function settings(){
 return `<section class="panel"><h2>⚙️ UI設定</h2><p class="small">LEAP全体の見た目をここから変更できます。</p>
...
function bind(){
 ...
 $("#createBattle")?.addEventListener("click",async()=>{
   ...
   if(!firebaseReady())return toast("UI設定からFirebaseを接続してください");
