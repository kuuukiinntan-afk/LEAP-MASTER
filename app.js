...
let cardFrom=1, cardTo=2300, cardOrder="seq";
function cards(){
...
function render(view="home"){
 ...
 const fn={home,cards,list,test,training,friends,battle,ranking,records,testRun}[view]||home;
...
function bind(){
 ...
 $("#applyCard")?.addEventListener("click",()=>{let a=+$("#cf").value||1,b=+$("#ct").value||2300;cardFrom=Math.min(a,b);cardTo=Math.max(a,b);cardOrder=$("#co").value;deck=W.filter(w=>w.id>=Math.min(a,b)&&w.id<=Math.max(a,b));if(cardOrder==="random")deck=shuffle(deck);if(cardOrder==="weak")deck=[...deck.filter(w=>S.weak[w.id]),...deck.filter(w=>!S.weak[w.id])];if(!deck.length)deck=W.slice();cardIndex=0;flipped=false;render("cards")});
...
if(!window.__LEAP_MENU_ROUTER__) {
  ...
  document.addEventListener("click", e => {
    ...
    if(view && typeof window.LEAPRender === "function") window.LEAPRender(view);
  }, true);
}
