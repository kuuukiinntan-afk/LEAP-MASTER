...
const menus = [
 ...
 ["battle","⚔️ オンライン対戦"],["ranking","🏆 ランキング"],["records","📅 学習記録"]
];
...
function menu(){
  ...
  const items=[
    ["home","⌂ ホーム"],["cards","🃏 単語カード"],["list","📚 単語一覧"],
    ["test","📝 テスト"],["training","⚡ トレーニング"],["friends","👥 フレンド"],
    ["battle","⚔️ オンライン対戦"],["ranking","🏆 ランク・ランキング"],
    ["records","📅 学習記録"],["settings","⚙️ 設定"]
  ];
...
let menuTouchX=0;
$("#sideMenu")?.addEventListener("touchstart",e=>{menuTouchX=e.changedTouches[0].clientX},{passive:true});
$("#sideMenu")?.addEventListener("touchend",e=>{const dx=e.changedTouches[0].clientX-menuTouchX;if(dx>55)closeMenu()},{passive:true});
document.addEventListener("keydown",e=>{if(e.key==="Escape")closeMenu()});

let deck=W.slice(), cardIndex=0, flipped=false, touchStartX=0, touchStartY=0, swiped=false;
let cardFrom=1, cardTo=2300, cardOrder="seq", cardSession=false, cardSessionCount=30, cardSessionDone=0;
function cards(){
  if(!cardSession){
    return `<section class="panel study cardSetup">
      <div class="eyebrow">FLASH CARDS</div><h2>🃏 単語カード</h2>
      <p class="small">学習する範囲と枚数を決めてから始めよう。</p>
      <div class="controls">
        <div class="field"><label>開始</label><input id="cf" type="number" min="1" max="2300" value="${cardFrom}"></div>
        <div class="field"><label>終了</label><input id="ct" type="number" min="1" max="2300" value="${cardTo}"></div>
        <div class="field"><label>順番</label><select id="co"><option value="seq" ${cardOrder==='seq'?'selected':''}>番号順</option><option value="random" ${cardOrder==='random'?'selected':''}>ランダム</option><option value="weak" ${cardOrder==='weak'?'selected':''}>苦手優先</option></select></div>
        <div class="field"><label>学習枚数</label><input id="cc" type="number" min="1" max="100" value="${cardSessionCount}"></div>
      </div>
      <button id="startCards" class="primary full">学習を始める</button>
    </section>`;
  }
  const total=Math.min(cardSessionCount,deck.length||1), w=deck[cardIndex%Math.max(1,deck.length)]||W[0];
  return `<section class="panel study cardSession">
    <div class="studyTop"><span>${Math.min(cardSessionDone+1,total)} / ${total}</span><span>${cardFrom}〜${cardTo}</span></div>
    <div class="progress"><i style="width:${Math.min(100,cardSessionDone/Math.max(1,total)*100)}%"></i></div>
    <div id="flashcard" class="flashcard ${flipped?'flipped':''}" tabindex="0" aria-label="単語カード。タップで裏返し、左スワイプで覚えてない、右スワイプで覚えた">
      <div class="flash-inner">
        <div class="flash-face"><span class="small">ENGLISH</span><div class="word">${esc(w.word)}</div><p>タップで答えを見る</p></div>
        <div class="flash-face flash-back"><span class="small">日本語</span><div class="meaning">${esc(w.meaning)}</div><p>右スワイプ：覚えた　・　左スワイプ：覚えてない</p></div>
      </div>
    </div>
    <div class="swipeHint"><span>← 覚えてない</span><span>右へ：覚えた →</span></div>
    <div class="actions"><button id="weak" class="weakBtn">← 覚えてない</button><button id="known" class="knownBtn">覚えた →</button></div>
    <button id="exitCards" class="textBtn">学習を終了</button>
  </section>`;
}
function list(){
...
let quiz=null;
function test(){
 return `<section class="panel testSetup">
   <div class="eyebrow">TEST</div><h2>📝 テスト</h2>
   <p class="small">2300語から範囲と出題形式を選んで挑戦。</p>
   <div class="testModes" role="group" aria-label="テスト形式">
    <button type="button" class="modeBtn active" data-test-mode="choice">四択</button>
    <button type="button" class="modeBtn" data-test-mode="jpen">日本語 → 英語</button>
    <button type="button" class="modeBtn" data-test-mode="enjp">英語 → 日本語</button>
    <button type="button" class="modeBtn" data-test-mode="mix">ミックス</button>
   </div>
   <div class="controls">
    <div class="field"><label>出題範囲・開始</label><input id="tf" type="number" min="1" max="2300" value="1"></div>
   ...
   <input id="tm" type="hidden" value="choice">
   <button id="startTest" class="primary full">テストを始める</button>
 </section>`;
}
function startTest(){
 ...
 const selectedMode=$("#tm").value||"choice";
 const items=pool.slice(0,n).map(w=>selectedMode==="mix"?{...w,testMode:Math.random()<.5?"jpen":"enjp"}:w);
 quiz={mode:selectedMode,items,i:0,ok:0,answered:false,results:[],left:Math.max(0,+$("#tmn").value||0)*60,timer:null};
...
function testRun(){
 ...
 const raw=quiz.items[quiz.i], mode=raw.testMode||quiz.mode, w=raw,q=mode==="enjp"?w.word:w.meaning,opts=mode==="choice"?choices():[];
 const body=mode==="choice"?`<div class="choices">${opts.map(x=>`<button data-choice="${esc(x)}">${esc(x)}</button>`).join("")}</div>`:`<div class="answer"><input id="answer" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="${mode==="enjp"?"日本語で答える":"英語で答える"}"><button id="submitAnswer" class="primary">回答</button></div>`;
 return `<section class="panel testRun"><div class="row"><span>${quiz.i+1} / ${quiz.items.length}</span><span>${quiz.left?Math.ceil(quiz.left)+"s":""}</span></div>
 <div class="progress"><i style="width:${quiz.i/quiz.items.length*100}%"></i></div><div class="small">${mode==="enjp"?"英語 → 日本語":mode==="jpen"?"日本語 → 英語":"4択"}</div>
 <div class="qword">${esc(q)}</div>${mode==="jpen"?`<div class="hint">💡 英語1文字目：<b>${esc(w.word[0].toUpperCase())}</b>…</div>`:""}${body}<div id="feedback"></div></section>`;
}
function answer(ans){
 if(quiz.answered)return;quiz.answered=true;
 const w=quiz.items[quiz.i],mode=w.testMode||quiz.mode,expected=mode==="enjp"?w.meaning:w.word;
 const ok=mode==="enjp"?answerMeaning(ans,expected):norm(ans)===norm(expected);
 ...
 if(mode==="choice")document.querySelectorAll("[data-choice]").forEach(b=>{b.disabled=true;if(b.dataset.choice===expected)b.classList.add("correct");if(b.dataset.choice===ans&&!ok)b.classList.add("wrong")});
...
function battle(){
  return `<section class="panel"><div class="row"><h2>⚔️ オンライン対戦</h2>${onlineStatus()}</div>
    ...
    <div class="notice">${firebaseReady()?"オンライン接続済み。ルームを作成するとフレンドへ招待が届きます。":"オンライン対戦を使うには、Firebase接続が必要です。"}</div>
  </section>`;
}
...
function settings(){
 const size=localStorage.getItem('leap_font_size')||'16';
 return `<section class="panel"><div class="eyebrow">SETTINGS</div><h2>⚙️ 設定</h2>
   <div class="settings"><div class="setting"><div><b>文字サイズ</b><div class="small">単語カードやテストの文字を調整</div></div><select id="fontSize"><option value="14" ${size==='14'?'selected':''}>小</option><option value="16" ${size==='16'?'selected':''}>標準</option><option value="18" ${size==='18'?'selected':''}>大</option></select></div>
   <div class="setting"><div><b>学習データ</b><div class="small">覚えた・苦手・学習記録は端末に保存されます。</div></div><button id="resetData" class="danger">データをリセット</button></div></div>
 </section>`;
}
...
function home(){
 ...
 return `<section class="homegrid">
   <div class="hero panel"><div class="heroCopy"><div class="eyebrow">LEAP 2300</div><h1>No LEAP,<br><span>No Life.</span></h1><p>2300語を覚えて、テストして、友達と競う。</p><div class="heroBtns"><button class="primary" data-go="cards">単語カードを始める</button><button data-go="test">テストする</button></div></div><div class="heroArt"><div class="orbit o1"></div><div class="orbit o2"></div><div class="bookArt"><div>LEAP</div><span>2300</span></div><i class="spark s1">✦</i><i class="spark s2">✧</i></div></div>
...
function bind(){
  const nav=$("#sideNav");
  // Menu navigation is handled once at document level below.
  // Keeping this function free of per-render menu handlers prevents stale handlers.
  if(nav) nav.setAttribute("data-ready","1");
 ...
 if(fc){
   ...
   fc.addEventListener("touchend",e=>{const t=e.changedTouches[0],dx=t.clientX-touchStartX,dy=t.clientY-touchStartY;if(Math.abs(dx)>55&&Math.abs(dx)>Math.abs(dy)){swiped=true;if(dx>0)markCard(true);else markCard(false);}},{passive:true});
   fc.addEventListener("keydown",e=>{if(e.key==="ArrowLeft")markCard(false);else if(e.key==="ArrowRight")markCard(true);else if(e.key==="Enter"||e.key===" "){e.preventDefault();flipped=!flipped;render("cards")}});
 }
 $("#startCards")?.addEventListener("click",()=>{let a=+$(`#cf`).value||1,b=+$(`#ct`).value||2300;if(a>b)[a,b]=[b,a];cardFrom=a;cardTo=b;cardOrder=$("#co").value;cardSessionCount=Math.min(100,Math.max(1,+$("#cc").value||30));deck=W.filter(w=>w.id>=a&&w.id<=b);if(cardOrder==="random")deck=shuffle(deck);if(cardOrder==="weak")deck=[...deck.filter(w=>S.weak[w.id]),...deck.filter(w=>!S.weak[w.id])];if(!deck.length)return toast("その範囲に単語がありません。");deck=deck.slice(0,cardSessionCount);cardIndex=0;cardSessionDone=0;flipped=false;cardSession=true;try{document.documentElement.requestFullscreen?.()}catch{}render("cards")});
 $("#known")?.addEventListener("click",()=>markCard(true));
 $("#weak")?.addEventListener("click",()=>markCard(false));
 $("#exitCards")?.addEventListener("click",()=>{cardSession=false;try{document.exitFullscreen?.()}catch{}render("cards")});
 function markCard(known){const w=deck[cardIndex%Math.max(1,deck.length)];if(!w)return;S.known[w.id]=!!known;S.weak[w.id]=!known;recordStudy(1);cardSessionDone++;if(cardSessionDone>=Math.min(cardSessionCount,deck.length)){cardSession=false;try{document.exitFullscreen?.()}catch{}toast("学習完了！");render("cards");return;}cardIndex=(cardIndex+1)%deck.length;flipped=false;render("cards")}
 $("#fontSize")?.addEventListener("change",e=>{localStorage.setItem('leap_font_size',e.target.value);document.documentElement.style.setProperty('--body-font',e.target.value+'px')});
 ...
 document.querySelectorAll("[data-test-mode]").forEach(b=>b.onclick=()=>{document.querySelectorAll("[data-test-mode]").forEach(x=>x.classList.remove("active"));b.classList.add("active");$("#tm").value=b.dataset.testMode;});
 ...
 $("#createBattle")?.addEventListener("click",async()=>{
   ...
   if(!firebaseReady())return toast("Firebase接続が必要です");
...
// Robust menu routing: one capture-phase handler works even after the menu HTML is re-rendered.
if(!window.__LEAP_MENU_ROUTER__) {
  window.__LEAP_MENU_ROUTER__ = true;
  document.addEventListener("click", e => {
    const b = e.target.closest?.("[data-view]");
    if(!b || !document.getElementById("sideNav")?.contains(b)) return;
    e.preventDefault();
    e.stopPropagation();
    const view = b.getAttribute("data-view");
    if(view && typeof window.LEAPRender === "function") { closeMenu(); window.LEAPRender(view); }
  }, true);
}
